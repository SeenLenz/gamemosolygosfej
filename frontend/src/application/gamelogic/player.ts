...
import { CollisionDir, DynamicGameObj, GameObject, ObjectTag } from "../base/gameobject";

export class Player extends DynamicGameObj {
    public focused: boolean = false;
    constructor(size: number[]) {
        super(new Vec2(size[0], size[1]), new Vec2(camera.zero.x + camera.width / 2, camera.zero.y + camera.height/2));
